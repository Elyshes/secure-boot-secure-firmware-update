...
#if defined(ENABLE_IMAGE_STATE_HANDLING) && defined(SFU_MPU_PROTECT_ENABLE)
#include "sfu_fwimg_state.h"
#endif /* ENABLE_IMAGE_STATE_HANDLING && SFU_MPU_PROTECT_ENABLE */
#include "string.h"
#include "mpu.h"
...
static SE_ErrorStatus FW_VALIDATE_GetActiveFwState(SE_StatusTypeDef *peSE_Status, uint32_t SlotNumber, SE_FwStateTypeDef *pFwState);
#endif /* ENABLE_IMAGE_STATE_HANDLING && SFU_MPU_PROTECT_ENABLE */
...
/**
 * @brief Validate a Active Firmware.
 * @param peSE_Status Status.
...
static SE_ErrorStatus FW_VALIDATE_ValidateFw(SE_StatusTypeDef *peSE_Status, uint32_t SlotNumber)
{
  ...
  SE_FwStateTypeDef current_fw_state;
  SE_FwStateTypeDef next_fw_state;
  uint32_t current_slot;

  /* Identify the concerned slot */
  if (SlotNumber == VALID_ALL_SLOTS)
  {
    next_fw_state = FWIMG_STATE_VALID_ALL;
    current_slot = MASTER_SLOT;
  }
  else
  {
    next_fw_state = FWIMG_STATE_VALID;
    current_slot = SlotNumber;
  }

  /* Get the active image state from RAM */
  e_ret_status = FW_VALIDATE_GetActiveFwState(peSE_Status, current_slot, &current_fw_state);

  if (e_ret_status == SE_SUCCESS)
  {
    /* Check that the current fw state is valid */
...
/**
 * @brief Retrieve an Active Firmware state.
 * @param peSE_Status Status.
 * @param SlotNumber index of the slot in the list
...
static SE_ErrorStatus FW_VALIDATE_GetActiveFwState(SE_StatusTypeDef *peSE_Status, uint32_t SlotNumber, SE_FwStateTypeDef *pFwState)
{
  ...
  *peSE_Status = SE_OK;
  ...
  {
    /* Translate the RAM state into SE state */
    switch (FWIMG_STATE[SlotNumber - SLOT_ACTIVE_1])
    {
    case FWIMG_STATE_TAG_INVALID:
    {
      *pFwState = FWIMG_STATE_INVALID;
      e_ret_status = SE_SUCCESS;
      break;
    }
    case FWIMG_STATE_TAG_VALID:
    {
      *pFwState = FWIMG_STATE_VALID;
      e_ret_status = SE_SUCCESS;
      break;
    }
    case FWIMG_STATE_TAG_VALID_ALL:
    {
      *pFwState = FWIMG_STATE_VALID_ALL;
      e_ret_status = SE_SUCCESS;
      break;
    }
    case FWIMG_STATE_TAG_SELFTEST:
    {
      *pFwState = FWIMG_STATE_SELFTEST;
      e_ret_status = SE_SUCCESS;
      break;
    }
    case FWIMG_STATE_TAG_NEW:
    {
  ...
  while (exit == 0U)
  {
    ...
    if (COM_Receive(&key, 1U, RX_TIMEOUT) == HAL_OK)
    {
      ...
      if (slot_number != 0U)
      {
        ...
        {
#if defined(SFU_MPU_PROTECT_ENABLE)
          if (slot_number == ALL_SLOTS)
          {
            FW_VALIDATE_GetActiveFwState(&se_Status, MASTER_SLOT, &fw_state);
          }
          else
          {
            FW_VALIDATE_GetActiveFwState(&se_Status, slot_number, &fw_state);
          }
#else  /* SFU_XXX_PROTECT_ENABLE */
          if (slot_number == ALL_SLOTS)
          {
            SE_APP_GetActiveFwState(&se_Status, MASTER_SLOT, &fw_state);
          }
          else
          {
          ...
          {
#if defined(SFU_MPU_PROTECT_ENABLE)
            se_retCode = FW_VALIDATE_ValidateFw(&se_Status, slot_number);
#else  /* SFU_XXX_PROTECT_ENABLE */
...
#else
    m_uDwlImgEnd = uFlashDestination + ((SE_FwRawHeaderTypeDef *)pData)->PartialFwSize + (((SE_FwRawHeaderTypeDef *)pData)->PartialFwOffset % SLOT_SIZE(SLOT_SWAP)) + SFU_IMG_IMAGE_OFFSET;
#endif /* SFU_NO_SWAP */
  }
